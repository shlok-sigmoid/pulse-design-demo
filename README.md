...
Pulse is a small brand-performance dashboard (static HTML/CSS/JS, no build step) used to show what the `design-plan` skill does. The skill ships inside this repo at `.claude/skills/design-plan/`, so it's active the moment you clone a branch and open Claude Code in the folder.

## The three branches

| Branch | What's in it | What to do with it |
|---|---|---|
| `pulse-before-design` | The dashboard built wrong on purpose — red everywhere, Arial, a narrow centered column, a spinner on the AI panel, marketing copy. | Run `/design-plan`, pick **Fix**. The skill audits every screen, lists the violations, and repairs them to match the design system. |
...
| `pulse-from-scratch` | Only a welcome page. No dashboard exists yet. | Ask Claude to build the Pulse dashboard. The skill generates it from the design system without a "before" to copy from. |
...
4. **From scratch.** Check out `pulse-from-scratch`, refresh, and show the welcome page. Ask Claude: "build the Pulse dashboard". The skill designs it from the document alone.
